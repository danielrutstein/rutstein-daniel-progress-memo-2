...
```
For anyone familiar with the NFL, the problem is obvious. Let's add year to this table.
...
```
Looks like a logarithmic regression to me. Let's normalize our w_av variable by an exponential approximation to minimize the affect of year possible as much as we can.
...
Viewing the density plot of `rel_w_av` and the boxplot of `rel_w_av` by `round`, the data behaves as we expect. Only 50.7% of players are "starters" for at least one NFL season, and only 22.9% are "starters: for at least 4 years (the duration of an NFL contract). Thus, it makes sense that the density of `rel_w_av` skews heavily to the right, since the near median draft pick does not even start for any season in the NFL. The long tail also makes sense since star players (Russell Wilson, Aaron Donald, Patrick Mahomes), return substantially more value to their teams over their careers than the average player. Grouping `rel_w_av` by round, the pattern also follows exactly what would be expected: each quartile and the median always decreases as round increases. The soundness of `rel_w_av` in conforming to expected patterns supports the notion that `rel_w_av` is a fairly accurate estimator of success of a draft pick.
```{r}
draft |>
...
## Analysis of relationship between Position Groups and rel_w_av
...
draft |>
  ggplot(aes(x = as.factor(year), y = rel_w_av)) +
  geom_boxplot() +
  facet_wrap(~pos_group) +
  labs(
    title = "Distribution of player value by year, grouped by position", 
    x = "round",
    y = "player value"
  )
